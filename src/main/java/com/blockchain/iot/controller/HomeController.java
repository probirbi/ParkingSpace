...
public class HomeController {
    ...
    // String[] broadCastUrls = {"http://localhost:8081/broadcast", "http://localhost:8083/broadcast"};
    ...
    public String temperature(HttpServletRequest request, Model model) {
        ...
        try {
            String url = "http://localhost:8081/temperature?requestedBy=ParkingSpaceNode";
            String result = "";
            ...
            if (entity != null) {
                ...
                newRatingBlock.setBlockCreatedBy("ParkingSpaceNode");
                newRatingBlock.setRating(rating);
                newRatingBlock.setRatingDoneBy("ParkingSpaceNode");

                System.out.println("rating block");
                try {
                    url = "http://localhost:8083/blockchain?create=true";
    ...
    public String smartHome(HttpServletRequest request, Model model) {
        ...
        // System.out.println("get data");
        try {
            String url = "http://localhost:8082/smarthome?requestedBy=ParkingSpaceNode";
            String result = "";
            HttpGet httpGet = new HttpGet(url);
    ...
    public String eHealth(HttpServletRequest request, Model model) {
        ...
        // System.out.println("get data");
        try {
            String url = "http://localhost:8085/ehealth?requestedBy=ParkingSpaceNode";
            String result = "";
            HttpGet httpGet = new HttpGet(url);
    ...
    public String energyConsumption(HttpServletRequest request) {
        ...
        try {
            String url = "http://localhost:8088/energyconsumption?requestedBy=ParkingSpaceNode";
            HttpGet httpGet = new HttpGet(url);
    ...
    private Block addInLocalBlockChain(Block block) {
        ...
        try {
            String url = "http://localhost:8083/addInLocalBlockChain";
    ...
    public String getTrustScore(@RequestParam(required = false) Integer node, Model model, HttpServletRequest request) throws JsonProcessingException {
        ...
        try {
            String url = "http://localhost:8083/blockchain";
            String result = "";
            HttpGet httpGet = new HttpGet(url);
        ...
        for (int i = 0; i < blockChainCopy.size(); i++) {
            if (blockChainCopy.get(i).getBlockType().equals(BlockType.RATING)) {
                if (blockChainCopy.get(i).getRatingDoneBy() != null && !blockChainCopy.get(i).getRatingDoneBy().equals("ParkingSpaceNode")) {
                    continue;
                }
        ...
        if (selectedNode > 0) {
            ...
            trustScoreBlock.setRatingDoneBy("ParkingSpaceNode");
            trustScoreBlock.setServiceRequestedBy("ParkingSpaceNode");
            ...
            try {
                String url = "http://localhost:8083/blockchain?create=false";
                HttpPost httpPost = new HttpPost(url);
                httpPost.setHeader("Content-type", "application/json");
                ObjectMapper mapper = new ObjectMapper();
    ...
    public String evaluate(@RequestParam String node, Model model) {
        ...
        try {
            String url = "http://localhost:8083/evaluatenode?node=" + nodeInt + "&nodeFrom=2";
            String result = "";
            //    System.out.println(url);
            HttpGet httpGet = new HttpGet(url);
    ...
    private void updateBlockchain(String hash, double rating) {
        try {
            String url = "http://localhost:8083/blockchain/updaterating";
            HttpPost httpPost = new HttpPost(url);
            httpPost.setHeader("Content-type", "application/json");
            String json = "{" +
                    "\"hash\":" + "\"" + hash + "\"," +
